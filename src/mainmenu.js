...
const Menu = (() => {
  ...
  /* ---------- open - BUG FIX v2.1 + PERFORMANCE v3.1: No lag, instant keys ---------- */
  function open() {
    // Instant UI first - no lag
    layer().hidden = false;
    ...
    // Focus first button instantly - keys work
    setTimeout(() => {
      const first = document.querySelector('#lobby-menu .menu-btn:not(:disabled)');
      if (first) first.focus();
    }, 30);

    // Deferred heavy audio - prevents lag and UI stuck
      ...
      setTimeout(() => {
        try {
          if (typeof Music !== 'undefined' && Music._lastMood !== 'menu') {
            ...
            console.log('[Menu] Menu music deferred - no lag');
  ...
  /* ---------- wiring ---------- */
  function init() {
    // BUG FIX v3.1: Single click handler only - prevents chapters double trigger return bug
    const handleAction = (e) => {
      ...
      if (chBtn) {
        e.preventDefault();
        e.stopPropagation();
        startChapter(Number(chBtn.dataset.chapter));
        return;
      }
      const vbtn = e.target.closest('#lv-btn');
      if (vbtn) {
        e.preventDefault();
        try { if (typeof TTS !== 'undefined') TTS.setEnabled(!TTS.isEnabled()); } catch (err) {}
      }
    };
    layer().addEventListener('click', handleAction);

    // BUG FIX: Focus speech with try/catch - prevents SFX error breaking keys
    layer().addEventListener('focusin', e => {
      const t = e.target.closest('[data-speak]');
      if (t) {
        try { if (typeof SFX !== 'undefined' && SFX.play) SFX.play('tick'); } catch (err) {}
        ...
        try { if (typeof TTS !== 'undefined') TTS.interrupt(t.dataset.speak, MENU_VOICE); } catch (err) {}
      }
    });
...
const CHAPTERS = {
  ...
  return {
    async start() {
      // BUG FIX v3.1: Lightweight, robust, no lag - boss ready
      try { init(); } catch (e) { console.error('Menu init failed', e); }
      try { await Game.boot(); } catch (e) { console.error('Game boot failed', e); }
      try { open(); } catch (e) { console.error('Menu open failed', e); }
      
      // Deferred heavy audio - after lobby visible, no lag
      setTimeout(() => {
        ...
        try { if (typeof BlindMusic !== 'undefined') { BlindMusic.init(); BlindMusic.playBase('noir_soft'); BlindMusic.setIntensity(0); } } catch (e) {}
